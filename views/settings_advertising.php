		...
		function echo_admob_ad_size($option_var_name, $default_value = 'medium') {
			...
			?><tr valign="top" class="ml_admob_ads_wrap">
				...
				<td class="ml_ad_interval">
					<?php
					$items = array( 'small' => array( 'Small', 100), 'medium' => array( 'Medium', 132), 'large' => array( 'Large', 320));
					?><select class="ml_admob_size_select" id="<?php echo esc_attr($option_var_name); ?>" name="<?php echo esc_attr($option_var_name); ?>">
						<?php foreach ( $items as $k => $value) {
							?><option
								value="<?php echo esc_attr( $k ); ?>" <?php selected($current_value, $k, true); ?> data-height="<?php echo esc_attr( $value[1] ); ?>">
								<?php echo esc_html( $value[0] ); ?>
							</option>
							<?php } ?>
					</select>
					<p>Please make sure the ad height in Admob is set to <span class="ml_admob_size_notice"><?php echo esc_attr( $items[$default_value][1] ); ?></span>dp</p>
				</td>
			</tr><?php
		}
		?>
